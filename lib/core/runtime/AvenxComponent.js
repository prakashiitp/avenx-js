...
export class AvenxComponent {
   ...
   * Prevents duplicate updates in the same microtask.
   */
  update() {
    ...
    try {
      ...
      if (this.#isMounted && this.#methods.onUpdate) {
        try {
          ...
          console.error(formatMessage(AvenxErrorCodes.LIFECYCLE_HOOK_ERROR, this.constructor.name, 'onUpdate', error));
        }
      }
    } finally {
  ...
  __afterMount() {
    ...
    if (this.#methods.onMount) {
      try {
        ...
        console.error(formatMessage(AvenxErrorCodes.LIFECYCLE_HOOK_ERROR, this.constructor.name, 'onMount', error));
      }
    }
  }
  ...
  unmount() {
    ...
    if (this.#methods.onUnmount) {
      try {
        ...
        console.error(formatMessage(AvenxErrorCodes.LIFECYCLE_HOOK_ERROR, this.constructor.name, 'onUnmount', error));
      }
    }
